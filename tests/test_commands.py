...
from mebot.cli.commands import app
from mebot.config.schema import Config
from mebot.providers.litellm_provider import LiteLLMProvider
from mebot.providers.openai_codex_provider import _strip_model_prefix
from mebot.providers.registry import find_by_model
...
def mock_paths():
    ...
    with patch("mebot.config.loader.get_config_path") as mock_cp, \
         patch("mebot.config.loader.save_config") as mock_sc, \
         patch("mebot.config.loader.load_config") as mock_lc, \
         patch("mebot.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "mebot is ready" in result.stdout
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "mebot.config.loader.set_config_path",
