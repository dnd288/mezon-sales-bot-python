"""Redis Streams bridge for external triggers and Mezon event forwarding."""
...
class RedisStreamsBridge:
    ...
    async def start(self) -> None:
        ...
        logger.info(
            "RedisStreamsBridge consuming stream={} group={} consumer={}",
    ...
    async def setup_event_forwarding(self, client: Any) -> None:
        ...
        for event_name in self.config.forward_events:
            if self._register_client_event(client, event_name):
                registered += 1
        logger.info("RedisStreamsBridge forwarding {} Mezon event types", registered)
        ...
        try:
            ...
            if target_type == "dm":
                ...
                if not content or not target:
                    logger.warning(
                        "Redis inbound rejected ({}): missing content/targetUserId for DM",
                        msg_id,
                    )
                    return
            else:
                ...
                if not content or not target:
                    logger.warning(
                        "Redis inbound rejected ({}): missing content/targetChannelId",
                        msg_id,
                    )
                    return
            ...
            if direct:
                logger.info("RedisStreamsBridge direct send to {}:{}", target_type, target)
    ...
    def _parse_json_obj(raw: Any) -> dict[str, Any]:
        """Parse a JSON object field if present, else return an empty dict."""
