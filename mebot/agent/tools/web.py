...
from mebot.agent.tools.base import Tool, tool_parameters
from mebot.agent.tools.schema import IntegerSchema, StringSchema, tool_parameters_schema
from mebot.utils.helpers import build_image_content_blocks

if TYPE_CHECKING:
    from mebot.config.schema import WebFetchConfig, WebSearchConfig
...
def _validate_url_safe(url: str) -> tuple[bool, str]:
    ...
    from mebot.security.network import validate_url_target
...
class WebSearchTool(Tool):
    ...
    def __init__(
        ...
        from mebot.config.schema import WebSearchConfig
...
class WebFetchTool(Tool):
    ...
    def __init__(self, config: WebFetchConfig | None = None, proxy: str | None = None, user_agent: str | None = None, max_chars: int = 50000):
        from mebot.config.schema import WebFetchConfig
        ...
        try:
            async with httpx.AsyncClient(proxy=self.proxy, follow_redirects=True, max_redirects=MAX_REDIRECTS, timeout=15.0) as client:
                async with client.stream("GET", url, headers={"User-Agent": self.user_agent}) as r:
                    from mebot.security.network import validate_resolved_url
